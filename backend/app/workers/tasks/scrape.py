...
enqueues scoring for each newly discovered job. Safe to re-run — JobIngestionService
skips already-known external ids before any detail fetch, and score_job_for_user is
itself idempotent (upsert on (user_id, canonical_job_id)). See docs/source-adapters.md.
"""
...
async def _run(source_name: str, keywords: list[str]) -> IngestionResult:
    ...
    async with session_scope() as session:
        ...
        user_id = await get_or_create_default_user_id(session)
