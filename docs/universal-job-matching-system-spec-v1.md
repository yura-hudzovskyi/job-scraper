...
|---|---|---|
| **0** — Baseline і вимірювання | **майже завершено** | ADR-файли під `docs/adr/` не написані (рішення зафіксовані тут, у 3.5). Seed-анотація (20.1): інструмент готовий і 300 пар набрано, лишилось судити — це людська робота |
...
| **4** — Taxonomy import and concept linker | **лексична частина + 9.4 завершені** | лишились concept embeddings і cross-encoder rerank (9.3 кроки 1 і 4). Перед реалізацією потрібен вимір 9.5 (див. нижче) |
| **5–10** | не почато | — |
...
**Internal concepts працюють (9.4, 2026-09-05).** Друга половина 9.4 —
та, що перетворює рішення рецензента на концепт — реалізована й перевірена на
проді. Промоушн створює концепт у namespace `internal` з типом `provisional`
і мітками під `und`; alias-індекс будується з двох namespace одразу, а ключ
кешу несе кількість internal-концептів, тому промоушн, зроблений в `api`,
доходить до кожного воркера без рестарту. Промоушн ідемпотентний за самим
терміном: два кліки не роблять двох концептів з однією міткою (це був би
штучний `ambiguous` із 9.3). Undo не лише знімає позначку, а й переводить
концепт у `retired` — інакше термін і далі лінкувався б на те, що рецензент
щойно відхилив.

Перевірено на реальних даних: 5 термінів (`Terraform`, `CI/CD`, `Linux`,
`Grafana`, `Prometheus`) → 5 концептів → `backfill.relink_profiles` за 44 с на
1985 профілях → 578 згадок перейшли з `unmapped` у `linked` у 563 документах.
Перелінкування свідомо не переекстракція: компетенції читаються зі збереженого
профілю, тому це словниковий пошук, а не 5 с моделі на документ.

**Форма черги, яка визначила дизайн.** 11 504 непокритих терміни, з них 8 162 —
рівно одна згадка (`texture resolution`, `domain expertise`), а топ-100 дає лише
16% згадок, топ-1000 — 47%. Тому: у черзі за замовчуванням поріг ≥2 згадки
(нічого не видаляється — термін виходить із хвоста, коли повторюється), і
bulk-рішення, бо немає короткого префікса, переглянувши який можна спинитись.
...
**Eval-набір: інструмент готовий, судження — за людиною (20.1, 2026-09-05).**
Побудовано все, крім самих міток: таблиця `evaluation_pairs`, стратифікований
семплер, метрики 20.4, шість ендпоінтів і екран анотації з клавішами 1–4.
Судження свідомо не генеруються — 20.1 прямо забороняє видавати згенеровані
оцінки за ground truth, і саме цей шар був би найлегшим місцем зрізати кут.

Набрано 300 пар seed-тиру з реального прода. Стратифікація зробила те, заради
чого існує: `uk` 154 проти `en` 145, попри корпус на 63% англійський, і всі
смуги балів представлені (0–10: 15, 30–40: 81, 40–50: 78, 50–60: 78, 60–70: 36,
70–80: 11). Пропорційна вибірка витратила б увесь бюджет у смузі 50–60 і не
сказала б нічого про те, де ранкер упевнений.

**Форма 20.1, яку тут неможливо виконати, і чому це чесно сказати.** Розділ
просить 50–100 профілів кандидатів і спліт за кандидатом проти витоку. Це
персональний інструмент з одним користувачем і двома CV — такого спліту не
існує і підробити його не можна. Лишається питання, яке продукт і ставить:
чи гарні вакансії вгорі для цього кандидата — і це рівно те, на чому 3.5.2
умова 3 тримає екстракцію. Подвійна анотація 15% (20.1) з одним анотатором теж
недосяжна; коли зʼявиться друга людина, колонка `annotator` уже це витримає.

**Перший baseline на реальних судженнях (20.4, 2026-09-05).** 624 оцінені
пари, 49 із них релевантні або сильні. Retrieval-only pipeline (Voyage embed +
rerank), без екстракції:

| Метрика | Значення | Що каже |
|---|---:|---|
| nDCG@5 | **0.925** | верх сторінки впорядкований майже ідеально |
| nDCG@10 | 0.787 | |
| P@10 | **1.00** | усі оцінені з топ-10 — релевантні |
| MRR@10 | **1.00** | перший же результат придатний |
| Recall@100 | **0.653** | **третина потрібного не потрапляє в топ-100** |
| Recall@200 | 0.796 | |

Діагноз однозначний і вказує на конкретну фазу: **верх ранжування відмінний,
хвіст губить вакансії**. Розділ 20.4 формулює це прямо — "a reranker cannot
recover a relevant job that retrieval never returned" — отже вузьке місце в
retrieval (Phase 5–6: представлення, польові вектори, hybrid + RRF), а не в
реранкері (Phase 7). Це той рідкісний випадок, коли eval-набір окупився з
першого застосування: без нього природним наступним кроком виглядав би
реранкер, і він не полагодив би нічого.

**Побічна знахідка: hard-фільтри ріжуть те, що користувач хоче.** Дві вакансії,
позначені `strong`, мають `eligible=false` і скор 0 через власне правило
зарплати ("salary tops out at 1000 USD, below desired 4000 USD"), тобто стоять
на ~1800 місці за побудовою. Загалом фільтри знімають 176 з 1934 вакансій, і
переважна більшість — саме за зарплатною стелею. Розділ 2.4 вимагає, щоб hard
filter умів показати речення, за яке він відхилив; тут він це вміє, і саме тому
видно, що поріг агресивніший за реальні наміри.

**Баг, який знайшла сама анотація (2026-09-05).** Вибір кандидата брав найсвіжіше
CV по всій інсталяції без прив'язки до користувача, а черга анотації не
фільтрувалась за кандидатом узагалі. З одним користувачем це невидимо; з двома —
друга людина, завантаживши CV, тихо перевела чужий eval на себе. 600 пар були
осуджені інженером-програмістом проти CV 3D-артиста, і 479 із них справедливо
дістали `irrelevant`. Виправлено на трьох рівнях (кандидат за користувачем,
черга за кандидатом, право судити за належністю пари), судження перенесені
задачею `evaluation.repoint_pairs` — вони пережили перенос, бо ніколи не
стосувалися CV: "чи хотів би я цю вакансію" — відповідь про анотатора й
вакансію.

**Реранкер не працював жодного разу (знайдено 2026-09-06).** Усі 1934 матчі
мали `relevance = NULL`. Кожен прогін слав 500 вакансій одним запитом, Voyage
відповідав 400, `_rerank` ловив виняток і повертав «немає оцінок», а пайплайн
спокійно ранжував на самій лише embedding-схожості — при `rerank_weight = 1.0`
у конфізі. Тобто **весь baseline 20.4, виміряний вище (nDCG@5 0.925,
Recall@100 0.576), це retrieval без реранкінгу взагалі.**

Ховали це дві речі. Виняток не ніс тіла відповіді, тож у логах був голий
`400 Bad Request`; тіло каже
`TOO_MANY_TOKENS_IN_BATCH: max allowed 600000, your batch has 931442`. І шлях
деградації мовчазний за задумом — що правильно для однієї збійної пачки і
неправильно для виклику, який падає завжди.

**Байти, а не символи.** Ті 500 вакансій — 1 548 088 символів, тобто ~387 000
токенів за звичним правилом «символи ÷ 4». Voyage нарахував 931 442. Кирилиця
коштує вдвічі-втричі більше, ніж припускає це правило, тож на корпусі, де 36%
українською, символьний бюджет помиляється більше за будь-який запас. Байтовий
безпечний у будь-якому письмі: byte-level BPE дає щонайбільше один токен на байт.

**Другий ліміт — швидкість, а не розмір.** rerank-3 дозволяє 2 000 000 токенів
за хвилину, а повний прохід по корпусу — ~2.9 млн. Жодний розмір пачки цього не
лікує; 429 треба перечекати. Це дешево: робота фонова, за чергою.

**Ліміти зняті, бо економити треба не на цьому.** `rerank_top_k = 0` означає
«всі придатні вакансії»: реранкер — єдина стадія, що читає CV і вакансію разом,
тож обмеження на нього обмежує якість ранжування, а не лише вартість. Платити за
це раз дозволяє кеш: оцінка залежить від CV, тексту вакансії й моделі, і два
хеші на рядку матчу кажуть, чи щось із цього зрушило. Інакше повний прохід
(~2.9 млн токенів) купувався б наново кожні 30 хвилин — 48 разів на добу за ту
саму відповідь (24.0 інваріант 4).

**Реранкер увімкнено — і eval-набір одразу спіймав регресію (2026-09-06).**
Після виправлення батчингу реранкер уперше відпрацював: 1758 із 1934 матчів
оцінені, relevance 0.262–0.809, перший повний прохід 240 с (із перечікуванням
rate-limit), другий — 8.9 с завдяки кешу. Верх ранжування виглядає правильно:
самі Full-Stack React+Python.

Але метрики впали:

| ваги | nDCG@5 | nDCG@10 | P@10 |
|---|---:|---:|---:|
| 0.0 (лише embedding) | **0.925** | **0.787** | **1.000** |
| 0.2 | 0.925 | 0.757 | 1.000 |
| 0.5 | 0.757 | 0.716 | 1.000 |
| 1.0 (лише реранкер) | 0.672 | 0.613 | 0.875 |

Монотонно: що більша вага, то гірше. Але **порівняння нечесне, і це важливіше
за самі числа**. Набір збирався семплером із embedding-ранжування, тож у топ-10
за вагою 0.0 оцінені всі 10 пар, а за вагою 1.0 — лише 8: дві вакансії реранкер
підняв, і їх ніхто не судив. nDCG виключає неоцінені (і має це робити), отже
рахує 8 пар проти 10. Це вимірює покриття, а не якість.

Реально спостережуване: одна справді слабка вакансія (мітка 1) на 7-му місці за
вагою 1.0. Решта різниці — методологія.

**Проміжне рішення — вага 0.2**: найбільша, за якої немає виміряної регресії
(20.6 забороняє релізити з регресією в критичному зрізі), і реранкер усе ж
працює на розрив нічиїх. Досемпльовано 120 пар із реранкованого ранжування, щоб
покрити його верх; після їх оцінки порівняння стане чесним і питання ваги
вирішуватимуть числа, а не обережність.

**Урок про сам набір.** Семплювати з одного ранжування й міряти ним інше —
приховане упередження, якого 20.1 прямо не називає. Кожна зміна ранжування
потребує свіжого семплу зі *свого* топу, інакше нове завжди виглядатиме гірше
за те, що набір і породило.
